...
- **Read from environment variables or a managed secret store — never commit keys.** Add `.env*` to `.gitignore`. Mirror the secret-loading pattern the rest of the codebase already uses; don't introduce a new one.
